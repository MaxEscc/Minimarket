...
namespace Minimarket
{
    public partial class Frm_Rubro : Form
    {
        ...
        private void Formato_rb()
        {
            ...
            dataGridViewListado.Columns[0].HeaderText = "CODIGO_RB";
            dataGridViewListado.Columns[1].Width = 300;
            dataGridViewListado.Columns[1].HeaderText = "RUBRO";
        }
        private void Listado_rb(string cTexto)
        {
            try
            {
                dataGridViewListado.DataSource = N_Rubro.Listado_rb(cTexto);
                this.Formato_rb();
            }
            catch (Exception ex)
            {
        ...
        {
            if (txt_Rubro.Text == String.Empty)
            {
            ...
            {
                E_Rubro oRb = new E_Rubro();
                string respuesta = "";
                oRb.codigo_rb = this.codigo_rb;
                oRb.descripcion_rb = txt_Rubro.Text.Trim();
                respuesta = N_Rubro.Guardar_rb(EstadoGuarda, oRb);
                if (respuesta == "OK")
                {
                    this.Listado_rb("%");
        ...
        {
            if (string.IsNullOrEmpty(Convert.ToString(dataGridViewListado.CurrentRow.Cells["codigo_rb"].Value)))
            {
        ...
        private void btn_eliminar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (Opcion == DialogResult.Yes)
                {
                    string  respuesta="";
                    this.codigo_rb = Convert.ToInt32(dataGridViewListado.CurrentRow.Cells["codigo_rb"].Value);
                    respuesta = N_Rubro.Eliminar_rb(this.codigo_rb);
        ...
        {
            Reportes.Frm_Rubro oReporteRb = new Reportes.Frm_Rubro();
            oReporteRb.txt_reportes.Text = dataGridViewListado.Text;
            oReporteRb.ShowDialog();
        ...
        {
            this.Listado_rb(txt_Buscar.Text.Trim());
        }

        //Refrescar los registros en el dataGrid
        private void btn_refrescar_Click(object sender, EventArgs e)
        {
            this.Listado_rb(dataGridViewListado.Text.Trim());
        }
